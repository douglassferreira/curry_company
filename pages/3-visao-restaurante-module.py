...
def avg_std_time_graph(df1):
    ...
    df_aux.columns = ['avg_time', 'std_time'] # mudando o nome das colunas
    df_aux = df_aux.reset_index()                
    fig = go.Figure()
...
def avg_std_time_delivery(df1, festival, op):
    """
    Esta função calcula calcula o tempo médio e o desvio padrão do tempo de entrega.
        Parâmetros:
            Input:
            ...
            Output:
                - df: Dataframe com os 2 colunas e 1 linha.
                    """
    ...
    df_aux = np.round(df_aux.loc[linhas, op], 2 )
...
def distance(df1, fig):
    if fig == False:
        ...
        df1['distance']=df1.loc[:, cols].apply( lambda x: haversine( (x['Restaurant_latitude'], x['Restaurant_longitude']),(x['Delivery_location_latitude'],x['Delivery_location_longitude'])), axis = 1 ) # axis=1 pq é coluna. Se fosse linha axis =0
